...
- **FR-016**: System MUST terminate the user session and redirect to the login page upon explicit logout. Upon receiving a 401 response (expired access token), the frontend MUST transparently attempt to refresh the token using the stored refresh token before redirecting to the login page. If the refresh succeeds, the original request is retried with the new access token. If the refresh fails, the session is terminated and the user is redirected to login.
